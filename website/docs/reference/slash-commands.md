...
| `/goal <text>` (alias: `/mission`) | Set a standing goal Hermes works toward across turns — our take on the Ralph loop and the OpenSession-style mission workflow. After each turn an auxiliary judge model decides whether the goal is done; if not, Hermes auto-continues. Subcommands include status, pause, resume, and clear. Budget defaults to 20 turns (`goals.max_turns`); state survives `/resume`. See [Missions and Persistent Goals](/user-guide/features/goals) for the full walkthrough and copyable examples. |
...
| `/agents` (aliases: `/tasks`, `/missions`) | Show active agents and running tasks across the current session. |
...
| `/goal <text>` (alias: `/mission`) | Set a standing goal Hermes works toward across turns. A judge model checks after each turn; if not done, Hermes auto-continues until it is, it needs you, you pause/clear it, or the turn budget is hit. `/mission resume` continues a paused human handoff. Safe to run mid-agent for status/pause/clear; setting a new goal requires `/stop` first. See [Missions and Persistent Goals](/user-guide/features/goals). |
...
| `/branch [name]` (alias: `/fork`) | Branch the current session (explore a different path). |
| `/agents` (aliases: `/tasks`, `/missions`) | Show active agents and running tasks. |
